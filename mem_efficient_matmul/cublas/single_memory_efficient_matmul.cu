...
torch::Tensor memory_efficent_matmul(torch::Tensor &mat_a, torch::Tensor &mat_b, int tile_size) 
{
    ...
    if(out_ptr_pad == nullptr) {std::cout << "Failed to allocate memory."<<std::endl; std::exit(-1);}
    float* a_ptr_pad = (float*)malloc(pad_m * pad_k *sizeof(float));
    if(a_ptr_pad == nullptr) {std::cout << "Failed to allocate memory."<<std::endl; std::exit(-1);}
    float* b_ptr_pad = (float*)malloc(pad_k * pad_n * sizeof(float));
    if(b_ptr_pad == nullptr) {std::cout << "Failed to allocate memory."<<std::endl; std::exit(-1);}
    ...
    CUDA_CHECK(cudaMemcpy2D(out_ptr_pad, pad_n*sizeof(float), out_ptr, n*sizeof(float), n*sizeof(float), m, cudaMemcpyHostToHost));
    CUDA_CHECK(cudaMemcpy2D(a_ptr_pad, pad_k*sizeof(float), a_ptr, k*sizeof(float), k*sizeof(float), m, cudaMemcpyHostToHost));
    CUDA_CHECK(cudaMemcpy2D(b_ptr_pad, pad_n*sizeof(float), b_ptr, n*sizeof(float), n*sizeof(float), k, cudaMemcpyHostToHost));
    ...
    for (int m_tile_idx=0; m_tile_idx<tile_num_m; m_tile_idx++)
    {
        ...
        {
            size_t offset_out = m_tile_idx * tile_size * pad_n + n_tile_idx*tile_size;
            for (int k_tile_idx=0; k_tile_idx<tile_num_k; k_tile_idx++)
            {

                size_t offset_a = m_tile_idx * tile_size * pad_k + k_tile_idx * tile_size;
                size_t offset_b = k_tile_idx * tile_size * pad_n + n_tile_idx * tile_size;
                // copy a tile of A to GPU
                CUDA_CHECK(cudaStreamWaitEvent(stream_a, event_out2host));
                CUDA_CHECK(cudaMemcpy2DAsync(a_dev, dpitch, (float*)a_ptr_pad+offset_a, hpitch_a, tile_size*sizeof(float), tile_size, cudaMemcpyHostToDevice, stream_a));
                CUDA_CHECK(cudaEventRecord(event_a, stream_a));

                // copy a tile of B to GPU
                CUDA_CHECK(cudaMemcpy2DAsync(b_dev, dpitch, (float*)b_ptr_pad+offset_b, hpitch_b, tile_size*sizeof(float), tile_size, cudaMemcpyHostToDevice, stream_b));
                ...
                CUDA_CHECK(cudaMemcpy2DAsync(out_dev, dpitch, (float*)out_ptr_pad+offset_out, hpitch_out, tile_size*sizeof(float), tile_size, cudaMemcpyHostToDevice, stream_out2dev));
                ...
                CUDA_CHECK(cudaMemcpy2DAsync((float*)out_ptr_pad+offset_out, hpitch_out, out_dev, dpitch, tile_size*sizeof(float), tile_size, cudaMemcpyDeviceToHost, stream_out2host));
    ...
    CUDA_CHECK(cudaMemcpy2D(out_ptr, n*sizeof(float), out_ptr_pad, pad_n*sizeof(float), n*sizeof(float), m, cudaMemcpyHostToHost));
