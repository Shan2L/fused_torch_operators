...
void PrepareAndLaunch(torch::Tensor& out, const torch::Tensor& in1, const Tensor& in2, int64_t tile_size, int64_t m, int64_t n, int64_t k)
{
    TORCH_INTERNAL_ASSERT(in1.scalar_type() == torch::ScalarType::Float, "Only support for float datatype now.");
...
torch::Tensor memory_efficient_matmul(const Tensor& in1, const Tensor& in2, const int64_t tile_size)
{
    ...
    TORCH_INTERNAL_ASSERT(size_in1.size() == 3, "The dimension of inpute tensor must be equal to 3");
    TORCH_INTERNAL_ASSERT(size_in2.size() == 3, "The dimension of inpute tensor must be equal to 3");
    TORCH_INTERNAL_ASSERT(size_in1[2] == size_in2[1], "The shape of input tensors does not match.");
    TORCH_INTERNAL_ASSERT(in1.device().is_cuda() && in2.device().is_cuda(), "The input tensor should be located on device.");
